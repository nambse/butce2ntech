...
export function DateRangePicker({
  ...
  const [uncontrolledDate, setUncontrolledDate] = React.useState<DateRange | undefined>(undefined)
  ...
  const handleSelect = (range: DateRange | undefined, presetLabel?: string) => {
    if (!range) {
      setInternalDate(undefined)
    ...
    if (range.from && range.to) {
      const orderedRange = {
        from: new Date(Math.min(range.from.getTime(), range.to.getTime())),
        to: new Date(Math.max(range.from.getTime(), range.to.getTime()))
      }
            ...
            {date?.from ? (
              selectedPreset ? (
                ...
                <>
                  ...
                  {format(date.to || date.from, "dd MMM yyyy", { locale: tr })}
                </>
              )
            ) : (
          ...
          <div className="p-2 border-b">
            <Select
              ...
              onValueChange={(value) => {
                ...
                if (preset) {
                  handleSelect(preset.getValue(), preset.label)
                }
              }}
              className="w-full"
